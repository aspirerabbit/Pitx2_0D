...
{       
   ...
   if (str3 == NULL){
	...
	sprintf(str3,"Pitx2_1_RA_%d.txt", bcl);
   ...
   if (strfmax == NULL){
     ...
     sprintf(strfmax,"Pitx2_1_APD_RA_%d.txt", bcl);
	...
	
{
	  ...
	  taufca= (80./(1+(CaSS/0.05)*(CaSS/0.05))+2.);
	   ...
	   fca = fcass-(fcass-fca)*exp(-dt/taufca);
		 ...
		 ibarca=0.00003980*(1+0.5*iso)*(1-0.5*AF);
...
void comp_iki ()
{
        gki = (1+1*AF)*0.09*pow(ko/5.4,0.4)*(1-PITX2*0.25);
...
void TPcai ()
{
	...
	Flecainide_DO=dFlecainide_DO+dFlecainide_DO;
		...
		if(Flecainide!=0.0)
	{
		Irel=(Vrel*(1.0/(1.0+Flecainide/55.0))*(sOO)*(CaSR-CaSS)+(1+0.25*AF)*Vleak*(sRR)*(1-1.0/(1.0+Flecainide/55.0))*(CaSR-CaSS));
	}
	else
	{
		Irel=(Vrel*(sOO)*(CaSR-CaSS)+(1+0.25*AF)*Vleak*(sRR)*(CaSR-CaSS));
	}
	
	//Irel=((1+PITX2*0.3)*Vrel*(sOO)*(CaSR-CaSS)+(1+0.25*AF)*Vleak*(sRR)*(CaSR-CaSS));
	
	//Irel=(Vrel*(sOO)*(CaSR-CaSS)+(1+0.25*AF)*Vleak*(sRR)*(CaSR-CaSS))*(1.0/(1.0+Flecainide/55.0));
...
void prttofile ()
{
//if((increment%1 == 0)&&t>(beats-2)*bcl)
