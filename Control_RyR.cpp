...
#define beats 90     /* Number of Beats */
#define S2  500
...
{       
   ...
   if (str3 == NULL){
	...
	sprintf(str3,"RyR_%d_PI_%g.txt",int(Flecainide),PITX2);
   ...
   if (strfmax == NULL){
	...
	sprintf(strfmax,"APDRyR_%d_PI_%g.txt",int(Flecainide),PITX2);
...
void comp_ina ()
{
        gna = (1+PITX2*0.1)*7.8*(1-AF*0.1)*(1.0/(1.0+Flecainide/84.0));//PITX2
	...
	
{
	  ...
	  taufca= (80./(1+(CaSS/0.05)*(CaSS/0.05))+2.);
	   ...
	   fca = fcass-(fcass-fca)*exp(-dt/taufca);
		 ...
		 ibarca=0.00003980*(1+0.5*iso)*(1-0.5*AF)*(1-PITX2*0.3);
...
void comp_iks ()
{
        gks = 0.129*(1+2.0*iso+1*AF)*(1+PITX2*1.5);
...
void comp_iki ()
{
        gki = (1+1*AF)*0.09*pow(ko/5.4,0.4)*(1-PITX2*0.3);
...
void TPcai ()
{
	...
	Flecainide_DO=dFlecainide_DO+dFlecainide_DO;
		...
		if(Flecainide!=0.0)
	{
	     Irel=((1+0.3)*Vrel*(1.0/(1.0+Flecainide/55.0))*(sOO)*(CaSR-CaSS)+(1+0.25*AF)*Vleak*(sRR)*(1-1.0/(1.0+Flecainide/55.0))*(CaSR-CaSS));
		//Irel=((1+PITX2*0.3)*Vrel*(1.0/(1.0+Flecainide/55.0))*(sOO)*(CaSR-CaSS)+(1+0.25*AF)*Vleak*(sRR)*(CaSR-CaSS));
	}
	else
	{
		Irel=((1+0.3)*Vrel*(sOO)*(CaSR-CaSS)+(1+0.25*AF)*Vleak*(sRR)*(CaSR-CaSS));
	}
...
//	Ileak=Vleak*(CaSR-Cai);////////////////
	Iup=(1+PITX2*1.0)*Vmaxup/(1.+((Kup*Kup)/(cai*cai)));///////////////
