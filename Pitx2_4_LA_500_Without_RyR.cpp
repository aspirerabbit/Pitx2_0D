...
#define beats 32     /* Number of Beats */
#define S2  500
...
void TPcai ()
{
	...
	Flecainide_DO=dFlecainide_DO+dFlecainide_DO;
		...
		if(Flecainide!=0.0)
	{
	     Irel=((1+PITX2*0.0)*Vrel*(1.0/(1.0+Flecainide/55.0))*(sOO)*(CaSR-CaSS)+(1+0.25*AF)*Vleak*(sRR)*(1-1.0/(1.0+Flecainide/55.0))*(CaSR-CaSS));
		//Irel=((1+PITX2*0.3)*Vrel*(1.0/(1.0+Flecainide/55.0))*(sOO)*(CaSR-CaSS)+(1+0.25*AF)*Vleak*(sRR)*(CaSR-CaSS));
	}
	else
	{
		Irel=((1+PITX2*0.0)*Vrel*(sOO)*(CaSR-CaSS)+(1+0.25*AF)*Vleak*(sRR)*(CaSR-CaSS));
	}
