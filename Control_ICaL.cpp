...
{       
   ...
   if (str3 == NULL){
	...
	sprintf(str3,"ICaL_%d_PI_%g.txt",int(Flecainide),PITX2);
   ...
   if (strfmax == NULL){
	...
	sprintf(strfmax,"APDICaL_%d_PI_%g.txt",int(Flecainide),PITX2);
	...
	
{
	  ...
	  taufca= (80./(1+(CaSS/0.05)*(CaSS/0.05))+2.);
	   ...
	   fca = fcass-(fcass-fca)*exp(-dt/taufca);
		 ...
		 ibarca=0.00003980*(1+0.5*iso)*(1-0.5*AF)*(1-0.3);
